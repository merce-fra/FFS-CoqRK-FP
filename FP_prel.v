...
Theorem error_sum_n_opt_bigop : forall {d} (v : 'cV_d.+1),
    format_mat v -> 
    let e:= \sum_(i < d.+1) (v i $0) in 
    let f:= \big[Rnd_Rplus / 0%R]_(i < d.+1) (v i $0) in 
    let a:= \sum_(i < d.+1) Rabs (v i $0) in 
...
replace (Rabs (\big[Rnd_Rplus/0]_(i < d.+1) v i $0 
             - \sum_(i < d.+1) v i $0))%R
   with (Rabs (f' - e')).
rewrite <- Hd at 1.
fold u in Hg.
...
Definition dot_product {d} (a b : 'cV[R]_d) W := 
           \big[(fun x y => W (x + y)) / 0%R]_(i < d) (W ((a i $0) * (b i $0))).
...
Lemma dot_product_error : forall {d} (a b : 'cV_d.+1), 
       ...
       <= INR d.+1 * u * (\sum_(i < d.+1) (Rabs (a i $0) * Rabs (b i $0))) 
        +  ((INR d.+1) ^ 2 * u + INR d.+1) */2 * eta.
Proof with auto with typeclass_instances.
intros d a b. 
apply Rle_trans with 
  (Rabs (dot_product a b r_flt 
        - \sum_i (r_flt (a i $0 * b i $0))) 
 + Rabs (\sum_i (r_flt (a i $0 * b i $0)) 
...
(* *)
instantiate (1 := 
   (u * (\sum_i (Rabs (a i $0) * Rabs (b i $0))) 
    + (INR d.+1) */2*eta)).
unfold dot_product.
eapply Rle_trans.
instantiate (1 := 
   (Rabs (\sum_i(r_flt(a i $0*b i $0) - a i $0*b i $0)))).
apply Req_le.
repeat f_equal.
...
lra.
apply Rle_trans with 
   (\sum_i (u * (Rabs (a i $0 * b i $0)) + /2 * eta)).
elim/big_ind2:_=> //.
...
instantiate (1 := 
  ((INR d.+1 - 1) * (u/(1+u)) *
     (\sum_(i < d.+1) ((1 + u)*Rabs (a i $0 * b i $0) 
     + /2*eta)))).
apply Rmult_le_compat_l.
apply Rmult_le_pos.
...
apply Rle_trans with 
 (Rabs (a i $0 * b i $0) + 
  ...
  + / 2 * eta)).
apply Rle_trans with 
  (Rabs (((\col_i0 r_flt (a i0 $0 * b i0 $0)) i $0) 
  - a i $0*b i $0)
 + Rabs (a i $0*b i $0)).
eapply Rle_trans. 
2 : apply Rabs_triang.
...
rewrite <- (Rmult_1_l (Rabs (a i $0 * b i $0))) at 1.
...
Proof.
intros d A v.
unfold mulmx_rnd, vec_norm at 1.
apply Rle_trans with 
...
apply Rle_trans with 
   ((INR d.+1 * u) * \big[Rplus/0]_(i0 < d.+1) 
        (Rabs ((row i A)^T i0 $0) * Rabs (v i0 $0)) 
   + ((INR d.+1) ^ 2 * u + INR d.+1) * /2 * eta).
apply Rle_trans with 
...
apply Rle_trans with 
     (\big[Rmax/0]_(i < d.+1) 
          \big[Rplus/0]_(k < d.+1) 
            (Rabs (A i k) * (\big[Rmax/0]_(z < d.+1) Rabs (v z $0)))).
elim/big_ind2: _ => //; try lra.
intros x1 x2 y1 y2 Hx Hy.
apply Rle_trans with (Rmax x2 y1).
now apply Rle_max_compat_l.
...
apply Rmult_le_compat_l.
apply Rabs_pos.
apply (@leq_bigmax _ (fun z => Rabs (v z $0))).
intros x; apply Rabs_pos.
apply Rle_trans with 
   (\big[Rmax/0]_(i < d.+1) 
         ((\big[Rmax/0]_(z < d.+1) Rabs (v z $0)) 
...
(** Bound on matrix-matrix product *)
Lemma mx_prod_error :forall {d} (A B : 'M_d.+1),
     ||| A (x)_m B - A *m B ||| <= (INR d.+1 * u) *||| A |||*||| B ||| 
      + INR d.+1 * (INR d.+1 ^ 2 * u + INR d.+1) * / 2 * eta.
Proof.
intros d A B. 
unfold matrix_norm at 1.
eapply Rle_trans with 
  (\big[Rmax/0]_(i < d.+1) (\big[Rplus/0]_(j < d.+1) 
            (INR d.+1*u*(\sum_(k < d.+1) (Rabs (A i k)*Rabs (B k j)))
                   + ((INR d.+1) ^ 2 * u + INR d.+1) */2 * eta))).
apply big_Rmax_le.
intros i.
elim/big_ind2:_=>//; try lra.
...
eapply Rle_trans.
apply Rle_trans with 
  (Rabs (dot_product (row i A)^T (col j B) r_flt 
       - dot_product (row i A)^T (col j B) (fun x => x))).
apply Req_le; f_equal.
transitivity (\big[Rnd_Rplus/0]_(j0 < d.+1) (A i j0 (x) B j0 j) 
            - \sum_(j0 < d.+1) A i j0 * B j0 j).
rewrite !mxE.
reflexivity.
f_equal.
unfold dot_product. 
...
apply Rle_trans with 
     ((\big[Rmax/0]_(i < d.+1) \big[Rplus/0]_(j < d.+1) 
      ((INR d.+1 * u) *\big[Rplus/0]_(k < d.+1) 
                 (Rabs (A i k) * Rabs (B k j)))) +
          INR d.+1 * (INR d.+1 ^ 2 * u + INR d.+1) * / 2 * eta).
rewrite <- big_Rmax_half_const.
apply  big_Rmax_le.
intros i.
rewrite big_Rplus_half_const; try easy.
apply Rplus_le_compat.
apply Rle_refl.
...
apply Rle_trans with ((INR d.+1 * u) * y1); try easy.
apply Rmult_le_compat_l.
repeat apply Rmult_le_pos; try lra.
apply pos_INR.
apply bpow_ge_0.
lra.
apply Rle_trans with ((INR d.+1 * u) * x1); try easy.
apply Rmult_le_compat_l.
repeat apply Rmult_le_pos; try lra.
apply pos_INR.
apply bpow_ge_0.
...
apply Rle_trans with 
    (\big[Rmax/0]_(i < d.+1) (\sum_(k < d.+1)
      Rabs (A i k) * \big[Rplus/0]_(j < d.+1) 
     (Rabs (B k j)))).
...
reflexivity.
apply Rabs_pos.
apply Rle_trans with (\big[Rmax/0]_(i < d.+1) 
       (\sum_(k < d.+1)
        (\big[Rmax/0]_(l < d.+1) (\big[Rplus/0]_(j < d.+1) 
          (Rabs (B l j)))) * Rabs (A i k))).
apply big_Rmax_le.
intros i.
...
apply Rmult_le_compat_r.
apply Rabs_pos.
apply (@leq_bigmax d.+1 
   (fun l => \big[Rplus/0]_(j0 < d.+1) Rabs (B l j0))).
intros l.
elim/big_rec:_=> //.
apply Rle_refl.
intros k x _ Hx.
...
easy.
apply Rle_trans with 
  (\big[Rmax/0]_(i < d.+1) ((\big[Rmax/0]_(l < d.+1) 
                         (\big[Rplus/0]_(j < d.+1) 
                         Rabs (B l j)))
          *(\sum_(k < d.+1) Rabs (A i k)))).
apply big_Rmax_le.
intros i.
rewrite big_Rplus_scal.
apply Rle_refl.
...
apply Rle_refl.
intros j x _ Hx.
destruct (Rle_dec (\big[Rplus/0]_(j0 < d.+1) 
                  Rabs (B j j0)) x).
rewrite Rmax_right.
easy.
easy.
...
intros i x _ Hx.
unfold Rmax;
destruct (Rle_dec (\big[Rplus/0]_(j < d.+1) 
                   Rabs (B i j)) x).
easy.
elim/big_rec:_=> //.
apply Rle_refl.
...
Lemma error_mat_const_prod_aux : Rabs (a' - a) <= B -> 
     ||| a' (x)_sm A -  a *_sm A ||| 
...
Lemma error_mat_const_prod : Rabs (a' - a) <= B 
     -> ||| A' - A ||| <= C * u  * ||| A ||| + D * eta
      -> ||| a' (x)_sm A' -  a *_sm A ||| 
...
eapply Rle_trans. 
apply Rplus_le_compat.
apply error_mat_const_prod_aux. 
apply Ha. 
instantiate (1 := 
 (Rabs a * (C * u * ||| A ||| + D * eta))).
eapply Rle_trans. 
...
Lemma bounds_alpha_bound_plus_flt : 
    \| X1 - A1 *m y \| <= C1 * vec_norm y + D1*eta ->
    \| X2 - A2 *m y \| <= C2 * vec_norm y + D2*eta -> 
    \| X1 (+)_m X2 - (A1 + A2) *m y \|
...
2 : apply vec_norm_pos.
2 : apply mx_vec_norm_submult.
rewrite <- mulmxA.
rewrite <- mulmxA.
rewrite <- mulmxBl.
apply mx_vec_norm_submult.
...
apply Req_le; ring.
eapply Rle_trans.
apply bounds_alpha_bound_plus_flt; try easy.
intros i j.
rewrite !mxE.
elim/big_ind:_=> //.
apply generic_format_0.
...
easy.
eapply Rle_trans. 
apply build_bound_mult_loc; try easy.
apply Rplus_le_compat.
fold K.
(* *)
apply Rmult_le_compat_r.
...
apply Rplus_le_compat.
replace (INR d.+1 ^ 2 * u) with 
   (INR d.+1 * INR d.+1 * u) by ring.
rewrite Rmult_assoc.
rewrite Rmult_comm.
apply Rmult_le_compat_r.
apply pos_INR.
